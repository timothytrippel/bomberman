	...
	switch (ivl_lpm_type(lpm)) {
		...
		case IVL_LPM_PART_PV:
			...
			if (ivl_lpm_q(lpm) == sink_nexus) {
				process_lpm_part_select(lpm, sink_nexus, sink_signal, ws);
			}
			break;
		// case IVL_LPM_CONCAT:
		// case IVL_LPM_CONCATZ: {
