...
def classify_counters(name, counters, constants, malicious, skipped, time_limit):
	for counter in counters:
		...
		if len(counter_value_set) == 1:
			if counter.name not in constants:
				if VERBOSE: print "Constant: " + counter.name
		...
		if len(counter_value_set) < max_possible_values:
			if counter.name not in malicious:
				if VERBOSE: print "Possible Malicious Symbol: " + counter.name
				malicious[counter.name] = True

	return CounterStats(name, counters, skipped, constants, malicious)
...
def main():
	...
	##
	# Iterate Over Simulation Time Intervals
	##
	for curr_time_limit in range(start_time, time_limit, time_resolution):
		...
		task_start_time    = time.time()
		dist_counter_stats = classify_counters("Distributed", dist_counters, {}, {}, {}, curr_time_limit)
		task_end_time      = time.time()
		calculate_and_print_time(task_start_time, task_end_time)
		print
		...
		task_start_time    = time.time()
		coal_counter_stats = classify_counters("Coalesced", coal_counters, {}, {}, {}, curr_time_limit)
		task_end_time      = time.time()
		calculate_and_print_time(task_start_time, task_end_time)
		print
