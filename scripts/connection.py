...
class Connection:
	...
	def __str__(self):
		out = self.source_sig.fullname() + "[" + str(self.source_msb) + ":" + str(self.source_lsb) + "]"
		out += " --> "
		out += self.sink_sig.fullname() + "[" + str(self.sink_msb) + ":" + str(self.sink_lsb) + "]"		
		return out

	@classmethod
	def from_signals(cls, sink_signal, source_signal):
		cls(sink_signal, sink_signal.msb, sink_signal.lsb, source_signal, source_signal.msb, source_signal.lsb)
