    ...
    */
    ...
    private void createSymbolTable()throws IOException
    {
        ...
        while((line = file.readLine()) != null)
        {
		    ...
		    if(SAVE_ALL_VALUES)
			signals.put(parts[3], new SignalSuspect(currentPath + "/", parts[4] + slice, st, bits, parts[3]));
		    else
    ...
    public void readValuesFromVCDSuspectsOnly()throws IOException
    {
        createSymbolTable();
        ...
        while((line = file.readLine()) != null)
        {
            ...
            else if(parts.length == 1)
            {
                ...
                Signal sig = signals.get(key);
                
                // Update the signal's value
                if(sig != null)
                {
                    sig.setValue("" + line.charAt(0), currentTime);
                
                    // Remove the signal if the added value remove the signal from suspicion
                    if(!((SignalSuspect)sig).isSuspect())
                        signals.remove(key);
                }
            }
            else if(parts.length == 2)
            {
                ...
                if(sig != null)
                {
                    sig.setValue(parts[0], currentTime);
