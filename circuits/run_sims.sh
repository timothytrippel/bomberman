	...
	if [[ $INTERACTIVE -eq 1 ]]; then
		...
		DOT_JOB_ID=$(\
			sbatch \
				...
				--constraint=opteron \
				--export=OUTPUT_DIR=LOGGING=${LOGGING},OUTPUT_DIR=${OUTPUT_DIR},DESIGN=${DESIGN},TTYPE=${TTYPE},CLK_BASENAME=${CLK_BASENAME} \
	...
	if [[ $INTERACTIVE -eq 1 ]]; then
		...
		VVP_JOB_ID=$(\
			sbatch \
				...
				--constraint=opteron \
				--export=OUTPUT_DIR=LOGGING=${LOGGING},OUTPUT_DIR=${OUTPUT_DIR},DESIGN=${DESIGN},TTYPE=${TTYPE} \
	...
	if [[ $INTERACTIVE -eq 1 ]]; then
		...
		VCD_JOB_ID=$(\
			sbatch \
				...
				--export=OUTPUT_DIR=LOGGING=${LOGGING},OUTPUT_DIR=${OUTPUT_DIR},DESIGN=${DESIGN},TTYPE=${TTYPE},NUM_TESTS=${NUM_TESTS} \
				--mem=${MEMORY} \
...
analyze_counters() {
	if [[ $INTERACTIVE -eq 1 ]]; then
		...
		sbatch \
			...
			--export=OUTPUT_DIR=LOGGING=${LOGGING},OUTPUT_DIR=${OUTPUT_DIR},DESIGN=${DESIGN},TTYPE=${TTYPE},START_TIME=${START_TIME},TIME_LIMIT=${TIME_LIMIT},TIME_RESOLUTION=${TIME_RESOLUTION} \
			--mem=${MEMORY} \
